...
export function EditarAsistencia() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        // Cargar asistencia específica
        const asistenciaResp = await fetch(`${appsettings.apiUrl}Asistencia/${id}`, {
        ...
        if (!asistenciaResp.ok || !estudiantesResp.ok || !gruposResp.ok) {
          throw new Error("Error al cargar los datos");
        }

        const asistenciaData: IAsistencia = await asistenciaResp.json();
        const estudiantesData = await estudiantesResp.json();
        const gruposData = await gruposResp.json();
        
        console.log("Asistencia cargada:", asistenciaData);
        ...
        setFormData({
          cod_Asistencia: asistenciaData.cod_Asistencia,
          cod_Estudiante: asistenciaData.cod_Estudiante.toString(),
          cod_Grupo: asistenciaData.cod_Grupo.toString(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const asistenciaData = {
        cod_Asistencia: formData.cod_Asistencia,
      ...
      console.log("Actualizando datos:", asistenciaData);

      const response = await fetch(`${appsettings.apiUrl}Asistencia/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`Error al actualizar asistencia: ${response.status} - ${errorText || 'Error desconocido'}`);
      }
    } catch (error) {
  ...
  if (loading) {
    return <div className="loading">Cargando datos de la asistencia...</div>;
  }
      ...
      <div className="form-container" style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '30px' }}>
            ...
            >
              {saving ? "Guardando..." : "Actualizar Asistencia"}
            </button>
          </div>
        </form>
      </div>
