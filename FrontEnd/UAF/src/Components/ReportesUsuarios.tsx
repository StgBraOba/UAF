...
export function ReporteUsuarios() {
  const [Usuarios, setEstudiantes] = useState<IUsuario[]>([]);
  ...
  const generarPDF = () => {
    ...
    doc.text("Reporte de Usuarios - UAF", 14, 15);

    // Mapeamos los datos para la tabla del PDF
    const tableData = Usuarios.map((est) => [
      est.username,
      est.id_Rol,
      est.email,
      est.estado
    ...
    autoTable(doc, {
      head: [["Username", "Rol","Email", "Estado"]],
      body: tableData,
    ...
    doc.save("Reporte_Usuarios_UAF.pdf");
  };
